...
library WadRayMath {
    ...
    function wadMul(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            if and(gt(y, 0), gt(x, div(MAX_UINT256_MINUS_HALF_WAD, y))) {
                revert(0, 0)
            }
    ...
    function wadDiv(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            if or(iszero(y), gt(x, div(sub(MAX_UINT256, div(y, 2)), WAD))) {
                revert(0, 0)
            }
    ...
    function rayMul(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            if and(gt(y, 0), gt(x, div(MAX_UINT256_MINUS_HALF_RAY, y))) {
                revert(0, 0)
            }
    ...
    function rayDiv(uint256 x, uint256 y) internal pure returns (uint256 z) {
        ...
        assembly {
            if or(iszero(y), gt(x, div(sub(MAX_UINT256, div(y, 2)), RAY))) {
                revert(0, 0)
            }
